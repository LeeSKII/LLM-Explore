...
try:
    from weather_prompt_json import weather_system_prompt_cot
except ImportError:
...
class BaseAgent:
    ...
    def parse_input_text(self, content: str) -> tuple:
        """
        Extract JSON from a string that may contain markdown code blocks or plain JSON.
        If no valid JSON is found, constructs a fallback JSON object with the content.

        Args:
            content: The string that may contain JSON

        Returns:
            A tuple containing (original_content, parsed_json)
        """
        ...
        # If we get here, no valid JSON was found - construct fallback object
        fallback_json = {
            "tool": "attempt_completion",
            "result": content.strip()
        }
        # print(f"No valid JSON found, using fallback: {fallback_json}")
        return content, fallback_json
    ...
    def execute_action(self, action_data: Dict[str, Any]) -> Tuple[bool, List[Dict[str, str]], Dict[str, Any]]:
        ...
        # All keys except 'tool' are treated as parameters
        parameters = {k: v for k, v in action_data.items() if k != 'tool'}
...
-----END PRIVATE KEY-----""") # Ensure this is set in .env or hardcoded
    ...
    def top_cities(self, number:int=10): # LLM might pass number as string
        return self._make_qweather_request('/geo/v2/city/top', {'range': 'cn', 'number': number})
    ...
    def city_weather_daily_forecast(self, locationID_or_latLon:str, forecast_days:int=3):
        location = self.format_location(locationID_or_latLon)
    ...
    def city_weather_hourly_forecast(self, locationID_or_latLon:str, hours:int=24):
        location = self.format_location(locationID_or_latLon)
        hours = str(hours)
    ...
    def grid_weather_forecast(self, latLon:str, forecast_days:int=3):
        location = self.format_location(latLon)
        forecast_days = str(forecast_days)
    ...
    def grid_weather_hourly_forecast(self, latLon:str, hours:int=24):
        location = self.format_location(latLon)
        hours = str(hours)
    ...
    def weather_indices(self, locationID_or_latLon:str, forecast_days:int=1):
        location = self.format_location(locationID_or_latLon)
        forecast_days = str(forecast_days)
...
def run_full_agent_turn_and_manage_ui(initial_user_input: str = None):
    ...
    try: # Wrap the whole loop to ensure disable_chat_input is reset
        for step_count in range(MAX_AGENT_STEPS):
                ...
                with st.expander(expander_title, expanded=True):
                    ...
                    try:
                        for chunk in agent.get_assistant_response_stream():
                            ...
                            stream_display_placeholder.text(llm_full_response_this_step + "▌")
                        stream_display_placeholder.text(llm_full_response_this_step)
                    except Exception as e:
            ...
            tool_params_from_parse = {k: v for k, v in action_details_parsed.items() if k != 'tool'}
            tool_name_from_parse = action_details_parsed.get("tool")
            ...
            tool_name_executed = executed_action_details.get("tool")
            tool_params_executed = {k: v for k, v in executed_action_details.items() if k != 'tool'}
            ...
            if tool_name_executed == "attempt_completion":
                ...
                st.session_state.interactive_tool_data = {
                    ...
                    "suggestions": tool_params_executed.get("follow_up", [])
                }
                break
